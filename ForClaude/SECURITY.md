...
- Ce projet a une seule table métier autoritaire pour les utilisateurs : `profiles`, avec `id uuid references auth.users(id) primary key` et une colonne `role` (`admin`, `responsable`, `agent`, `lecteur`). `auth.users` reste réservé à l'identité/authentification (email, mot de passe).
- Ne jamais créer ou réutiliser une table `users`/`utilisateurs` parallèle à `profiles` — c'est une source de bugs de RLS garantie (double source de vérité pour le rôle). Si une telle table existe encore dans le projet, la fusionner dans `profiles` plutôt que la faire évoluer.
- Toute vérification de rôle dans une policy RLS doit passer par la fonction `security definer` `current_user_role()` (à créer si absente) plutôt que par une sous-requête directe sur `profiles`, pour éviter la récursion RLS :
...
- Le rôle d'un utilisateur (colonne `profiles.role`) ne doit jamais être modifiable par l'utilisateur lui-même via une policy `UPDATE` générique sur `profiles` — prévoir une policy `WITH CHECK` qui exclut explicitement la colonne `role`, ou restreindre sa modification aux seuls `admin`.
...
- Chaque nouvelle table avec RLS doit définir explicitement une policy par opération (`SELECT`, `INSERT`, `UPDATE`, `DELETE`) alignée sur les rôles `admin`, `responsable`, `agent`, `lecteur` — ne jamais partir du principe qu'une policy `SELECT` couvre implicitement les autres opérations.
